...
export default function Success() {
  ...
  // const localData = localStorage.getItem('bookingData');
      ...
      const totalPaid = confirmation?.TotalPrice || bookingData?.rooms?.reduce((sum, room) => sum + (parseFloat(room.TotalPrice) || 0), 0) || 0;
 ...
 <span id="rewardId" style={{ display: "inline-block", marginTop: 56, marginLeft:30, paddingLeft:20 }}>
    ...
    </span>                        <Typography variant="h4" fontWeight="600" id="rewardId" textAlign="center">
    Booking Confirmed!
                        </Typography>
                    ...
                    <Box sx={{ flex: 1.5, p: { xs: 4, md: 6 }, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                        ...
                        {datas ? (
                            <Stack spacing={2}>
                                <Typography>
                                    <strong>Hotel:</strong> {datas?.hotel.name || datas.hotel?.name || datas.hotel?.name}
                                </Typography>
                                ...
                                <Box>
                                    ...
                                    <Box sx={{ pl: 2, p: 1.5, bgcolor: 'grey.50', borderRadius: 1 }}>
                                        <Typography variant="body1" fontWeight={600}>
                                            {datas.rooms.length} × {datas.rooms[0]?.Description || datas.rooms[0]?.Rooms?.[0] || 'Standard Room'}
                                        </Typography>
